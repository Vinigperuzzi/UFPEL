...
node *reset(void);
void push(node *novo, int chave, node *pai);
...
node *reset(void)//Cria um novo elemento para ser inserido. (Usada pela função push)
{
...
void push(node *no, int chave, node *pai)//Insere um novo nó na árvore. (deve ser chamada passando o ponteiro da raiz e o valor da chave)
{
	if (no == NULL){
		no = reset();
		if (no == NULL)//Return case malloc fails
			return;
		no->chave = chave;
		no->cor = 1;
		if (raiz == NULL)//refresh the root case it was the first insertion
			raiz = no;
		no->pai = pai;
		if (no->pai != NULL){
	...
	if (no->chave > chave){
		push(no->esq, chave, no);
	}
	else if (no->chave < chave){
		push(no->dir, chave, no);
	}
	else
